<template>
    <div class="dashboard-container">
        ...
        <div class="cards-wrapper">
            <!-- 👤 User Status Cards -->
            <div class="card" v-for="(count, status) in userStats" :key="status" :class="statusColor(status)">
                <h2>{{ status }}</h2>
                <p>{{ count }} Users</p>
            </div>

            <!-- 🏦 Account Status Cards -->
            <div class="card" v-for="(count, status) in accountStats" :key="status" :class="statusColor(status)">
                <h2>{{ status }}</h2>
                <p>{{ count }} Accounts</p>
            </div>
        </div>
